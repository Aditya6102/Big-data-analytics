...
data <- as.data.frame(cbind(First,Second)) # binding function for data frame
...
data("anscombe")
anscombe
# generate levels for indicating group number
...
data(mtcars)
dotchart(mtcars$mpg, labels=row.names(mtcars), cex=0.6, main="Miles per
         Gallons of Car models", xlab="mpg")
barplot(table(mtcars$cyl), main="Dist of Car Cylinder Count", xlab="Number of
        cylinders")
...
library("ggplot2")
data(diamonds)
#only including premium and ideal diamonds
...
# creating matrix using sticking technique
r1 <- c("I", "am","happy")
r2 <- c ("what", "a", "day")
r3 <-1:3
...
D <-cbind(r1,r2,r3) #becomes a column
D
...
FieldGoals
# Approximate number of goals/ game (rounded to 2 decimal points)
...
?matplot
FieldGoals
t(FieldGoals) # used to transpose matrix
...
Games
Games[1:3, 6:10]
Games[c(1,10), ] # first and last row,including all columns
Games[ , c("2008","2009")] # all players columns from 2008 and 2009
Games[1,] # one row, a vector
Games[1,5] # A vector, too.
is.matrix(Games[1,])
is.vector(Games[1,])
# R automatically guessed you needed a vector, you can stop it
Games[1, , drop=F] # now it stays as matrix, not vector
...
myplot2(Points)
# in-game metrics  normalized( for reduction of injury skewed results)
